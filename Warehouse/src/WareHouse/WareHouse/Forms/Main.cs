...
namespace WareHouse.Forms
{
    public partial class Main : Form
    {
        ...
        private void SaveLoadButton_Click(object sender, EventArgs e)
        {
            try
            {
                using (var saveLoad = new SaveLoadForm())
                {
                    saveLoad.ShowDialog();

                    if (!saveLoad.IsLoad) return;
                    Manager.Clear();
                    Manager.Load(saveLoad.LoadFolder);
                    Manager.Save();
                    Application.Restart();
                }
            }
        ...
        private void AddProduct()
        {
            while (true)
            {
                ...
                {
                    if (MessageBox.Show(exception.Message, ApplicationStrings.ErorrMessage, MessageBoxButtons.RetryCancel,
